...
class fault_injection:
    ...
    def declare_weight_fi(self, **kwargs):
        ...
        if kwargs:
            if "function" in kwargs:
                ...
                #custom
                corrupt_layer_v = kwargs.get("conv_num", -1)
                corrupt_k = kwargs.get("k", -1)
                corrupt_c = kwargs.get("c", -1)
                corrupt_kH = kwargs.get("h", -1)
                corrupt_kW = kwargs.get("w", -1)
        ...
        #Support for multiple injections as a list

        if type(corrupt_layer_v) == list:

            for i in range(len(corrupt_layer_v)):
                #print("\r",'Injection: ', i, end='')
                corrupt_layer=corrupt_layer_v[i]
                ...
                for name, param in self.CORRUPTED_MODEL.named_parameters():
                    ...
                    if "conv" in name or "shortcut.0" in name or "downsample.0" in name:
                        ...
                        if curr_layer == corrupt_layer:
                            ...
                            #custom
                            #print("modified")
                            param.data[corrupt_idx] = corrupt_value[i]
                            # program was halting on logging
    ...
    def declare_weight_bit_fi(self, **kwargs):
        self._fi_state_reset()
        CUSTOM_INJECTION = False
        CUSTOM_FUNCTION = False
        ...
        if kwargs:
            if "function" in kwargs:
                ...
                corrupt_kW = kwargs.get("w", -1)
                corrupt_value = kwargs.get("value", -1)
                bits = kwargs.get("bits", -1) # random 64 bit float
        else:
            raise ValueError("Please specify an injection or injection function")
        ...
        if type(corrupt_layer_v) == list:
            ...
            for i in range(len(corrupt_layer_v)):
                #print("\r",'Injection: ', i, end='')
                corrupt_layer=corrupt_layer_v[i]
                bit=bits #Change to bits[i] for custom bits
                corrupt_idx = [corrupt_k[i], corrupt_c[i], corrupt_kH[i], corrupt_kW[i]]
                ...
                for name, param in self.CORRUPTED_MODEL.named_parameters():
                    ...
                    if "conv" in name or "shortcut.0" in name or "downsample.0" in name:
                        ...
                        if curr_layer == corrupt_layer:
                            ...
                            #custom
                            #print("modified")
                            corrupt_value= torch.tensor(bit_flip_f64(orig_value,bit))
                            param.data[corrupt_idx] = corrupt_value
                            # program was halting on logging
                            # logging.info("Weight Injection")
                            # logging.info("Layer index: %s" % corrupt_layer)
    ...
    def declare_weight_fi_fc(self, **kwargs):
        self._fi_state_reset()
        CUSTOM_INJECTION = False
        CUSTOM_FUNCTION = False
        ...
        if kwargs:
            if "function" in kwargs:
                ...
                corrupt_kW = kwargs.get("w", -1)
                corrupt_value = kwargs.get("value", -1)
                corrupt_fcx_v = kwargs.get("fcx_v", -1)
                corrupt_fcy_v = kwargs.get("fcy_v", -1)
        else:
            raise ValueError("Please specify an injection or injection function")
                    ...
                    if "conv" in name or "shortcut.0" in name or "downsample.0" in name or "fc.weight" in name:
                        ...
                        if curr_layer == corrupt_layer:
                            ...
                            if "fc.weight" in name:
                                corrupt_idx=[corrupt_fcx_v[i],corrupt_fcy_v[i]]
                                corrupt_idx = (
                                tuple(corrupt_idx)
                                if isinstance(corrupt_idx, list)
                                else corrupt_idx
            ...
            for name, param in self.CORRUPTED_MODEL.named_parameters():
                ...
                if "conv" in name or "shortcut.0" in name or "downsample.0" in name or "fc.weight" in name:
                    ...
                    if curr_layer == corrupt_layer:
                        ...
                        if "fc.weight" in name:
                            corrupt_idx=[corrupt_fcx_v,corrupt_fcy_v]
                            corrupt_idx = (
                            tuple(corrupt_idx)
                            if isinstance(corrupt_idx, list)
                            else corrupt_idx
